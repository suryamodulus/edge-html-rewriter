...
export class ParserFeedbackSimulator {
  ...
  onWhitespaceCharacter(token) {
    if (
      ...
      token.chars.charCodeAt(0) === LINE_FEED_CODE_POINT
    ) {
      this.skipNextNewLine = false;
  ...
  onStartTag(token) {
    ...
    if (this.inForeignContent) {
      if (foreignContent.causesExit(token)) {
        ...
        if (
          !token.selfClosing &&
          foreignContent.isIntegrationPoint(tn, currentNs, token.attrs)
        ) {
          this._enterNamespace(html.NS.HTML);
        }
      }
  ...
  onEndTag(token) {
    ...
    if (!this.inForeignContent) {
      ...
      if (previousNs === html.NS.SVG) {
        const adjustedTagName = foreignContent.SVG_TAG_NAMES_ADJUSTMENT_MAP.get(
          token.tagName,
        );
        if (adjustedTagName) {
